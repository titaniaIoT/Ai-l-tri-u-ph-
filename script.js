...
function show_notification_confirm(message) {
  ...
  const notification_message = document.getElementById(
    "notification_message_confirm"
  );
  notification_message.innerText = message;
  notification.classList.remove("hidden");
}
...
let question_award = [];
for (let i = 1; i <= 15; i++) {
  question_award.push(i.toString());
}

let get_money_award = [];
for (let i = 1; i <= 15; i++) {
  get_money_award.push(
    (i * 1000000)
...
function display_question(question_data) {
  ...
  question_data.answers.forEach((answer) => {
    ...
    button.onclick = () =>
      select_answer(button, answer, question_data.correctAnswer);
...
function start_countdown() {
  ...
  function update_timer() {
    ...
    if (time_left <= 0) {
      ...
      const notification_button = document.querySelector(
        ".notification button"
      );
...
// Nút hỏi ý kiến khán giả
function use_audience() {
  const correct_answer = get_correct_answer();
  console.log("Correct answer:", correct_answer);
  const total_audience = 100;
  const votes = audience_votes(total_audience, correct_answer);
  console.log("Votes:", votes);
  show_audience_chart(votes);
  disable_button(audience_button);
}

function audience_votes(total, correctAnswer) {
  const votes = {
    A: 0,
    B: 0,
    C: 0,
  ...
  for (let i = 0; i < total; i++) {
    let answer = i === 0 ? correctAnswer : get_random_answer();
    votes[answer]++;
  }
  return votes;
}
...
let audiences_chart = null;

function show_audience_chart(votes) {
  const canvas = document.getElementById("audience_chart");
  if (audiences_chart) {
    audiences_chart.destroy();
  }
  const ctx = canvas.getContext("2d");
  ctx.clearRect(0, 0, canvas.width, canvas.height);
  const labels = ["A", "B", "C", "D"];
  const data = labels.map((label) => votes[label]);
  const chartConfig = {
    type: "bar",
    data: {
      labels: labels,
  ...
  audiences_chart = new Chart(ctx, chartConfig);
  const audience_dialog = document.getElementById("audience");
  audience_dialog.classList.remove("hidden");
}
function close_audience() {
  const audience_dialog = document.getElementById("audience");
  audience_dialog.classList.add("hidden");
}
...
function update_money(correct_count, incorrect_count, time_out) {
  ...
  if (money_element && money_element_fix) {
    if (incorrect_count == 1) {
      money_element.textContent = `Số tiền bạn nhận được: 0₫`;
      money_element_fix.textContent = `Số tiền bạn nhận được: 0₫`;
    } else {
      if (time_out == 1) {
        money_element.textContent = `Số tiền bạn nhận được: 0₫`;
        money_element_fix.textContent = `Số tiền bạn nhận được: 0₫`;
      } else {
        money_element.textContent = `Số tiền bạn nhận được: ${
          get_money_award[correct_count - 1]
        }`;
        money_element_fix.textContent = `Số tiền bạn nhận được: ${
          get_money_award[correct_count - 1]
        }`;
      }
    }
...
function show_result_screen() {
  update_money(correct_count, incorrect_count, time_out);
  ...
  document.getElementById(
    "correct_count"
  ).innerText = `Số câu đúng: ${correct_count}`;
  document.getElementById(
    "incorrect_count"
  ).innerText = `Số câu sai: ${incorrect_count}`;
  document.getElementById(
    "notanswer_count"
  ).innerText = `Số câu chưa làm: ${notanswer_count}`;
...
function restart_game() {
  ...
  var level_screen_element = document.getElementById("login_screen");
  level_screen_element.style.display = "block";
  current_question_index = 0;
  correct_count = 0;
  incorrect_count = 0;
  ...
  close_audience(); // Đảm bảo rằng hộp thoại audience được đóng lại khi chơi lại
}

// Hàm vẽ biểu đồ tròn-----------------------------------------------------------------------------------------------
function draw_pie_chart(correct_count, incorrect_count, notanswer_count) {
  const result_canvas = document
    .getElementById("result_chart")
    .getContext("2d");
  if (result_canvas.chart) {
    result_canvas.chart.data.datasets[0].data = [
      correct_count,
      incorrect_count,
      notanswer_count,
    ];
...
function display_admin_data(questions) {
  ...
  questions.forEach((question, index) => {
    ...
    question_div.innerHTML = `
              ...
              <p class="question_text"><strong>Câu hỏi ${index + 1}:</strong> ${
      question.question
    }</p>
              ...
              <button onclick="delete_question_confirm('${
                question.id
              }')">Xóa câu hỏi</button>
              <button onclick="edit_question('${
                question.id
              }')">Chỉnh sửa</button>
          `;
  ...
  questions.forEach((question) => {
    ...
    if (question_text_element) {
      ...
      if (question_text.includes(search)) {
        ...
        const highlighted_text = question_text_element.innerHTML.replace(
          regex,
...
function scroll_to_result(index) {
  ...
  search_results.forEach((element) =>
    element.classList.remove("current_result")
  );
  current_index = index;
  search_results[current_index].classList.add("current_result");
  search_results[current_index].scrollIntoView({
    behavior: "smooth",
    block: "center",
  });
}
...
function edit_question(id) {
  const question_element = document.querySelector(
    `.question_item[data-id="${id}"]`
  );
  const question_text = question_element
    .querySelector("p.question_text")
    .textContent.replace(/Câu hỏi \d+:/, "")
    .trim();
  const answers_list = Array.from(
    question_element.querySelectorAll("ul li")
  ...
  for (let i = 0; i < childNodes.length; i++) {
    ...
    if (
      node.nodeName === "P" &&
      node.textContent.trim().startsWith("Đáp án đúng:")
    ) {
  ...
  if (correct_answer_element) {
    const correct_answer = correct_answer_element.textContent
      .trim()
      .replace("Đáp án đúng:", "");
    ...
    edit_form.innerHTML = `
              ...
              <input type="text" id="edit_answer_1_${id}" value="${
      answers_list[0]
    }">
              <label for="edit_answer_2_${id}">Đáp án B:</label>
              <input type="text" id="edit_answer_2_${id}" value="${
      answers_list[1]
    }">
              <label for="edit_answer_3_${id}">Đáp án C:</label>
              <input type="text" id="edit_answer_3_${id}" value="${
      answers_list[2]
    }">
              <label for="edit_answer_4_${id}">Đáp án D:</label>
              <input type="text" id="edit_answer_4_${id}" value="${
      answers_list[3]
    }">
              <label for="edit_correct_answer_${id}">Đáp án đúng:</label>
              <select id="edit_correct_answer_${id}" >
                  <option value="${answers_list[0]}" ${
      correct_answer === answers_list[0] ? "selected" : ""
    }>A</option>
                  <option value="${answers_list[1]}" ${
      correct_answer === answers_list[1] ? "selected" : ""
    }>B</option>
                  <option value="${answers_list[2]}" ${
      correct_answer === answers_list[2] ? "selected" : ""
    }>C</option>
                  <option value="${answers_list[3]}" ${
      correct_answer === answers_list[3] ? "selected" : ""
    }>D</option>
              </select>
...
async function save_edit_question(id) {
  ...
  const edited_correct_answer = document.getElementById(
    `edit_correct_answer_${id}`
  ).value;
...
function log_out_confirm() {
  show_notification_confirm("Bạn chắc chắn muốn đăng xuất?");
  const no_button = document.getElementById("No");
  const yes_button = document.getElementById("Yes");
